...
########################################################
#####Watershed
cap = cv2.VideoCapture('01_early_finisher_high_activity_day_seg.mp4')
count = 0
while(cap.isOpened()):
    ret, frame = cap.read()
    ...
    print(frame.shape)
    img = frame
    #img = cv2.imread('frame1204.jpg')  #Read as color (3 channels)
    img_grey = img[:,:,0]
    ...
    #cv2.imshow('watershed', markers_show)
    cv2.imshow('gray_area', unknown)
    cv2.imshow('gray', sure_bg)
